...
void LCD_Custom_Char(unsigned char loc, const unsigned char *bitmap) {
    unsigned char i;
    if(loc < 8) {
        // La dirección base de la CGRAM empieza en 0x40
        // Cada carácter ocupa 8 bytes de memoria
        LCD_Cmd(0x40 + (loc * 8)); 
        
        for(i = 0; i < 8; i++) {
            *(lcd.PORT) |= 1 << lcd.RS;   // RS = 1 para enviar datos
    ...
    // Regresar el LCD al modo de visualización normal (DDRAM)
    LCD_Cmd(0x80); 
}