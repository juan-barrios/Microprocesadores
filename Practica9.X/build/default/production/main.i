...
volatile unsigned int pos[2] = {0,0}, lastPos[2];
...
void whereToMove(unsigned int adc1, unsigned int adc2, volatile unsigned int pos[]){
...
char animation(char estado){
    if(estado == 0) return 1;
    return 0;
}

void show(int pos[]){
    do { if ( pos[0] == 0 ) { do { LCD_Write( (0x80 + pos[1] & 0xF0) >> 4 ); LCD_Write( 0x80 + pos[1] & 0x0F); } while ( 0 ); } else if ( pos[0] == 1 ) { do { LCD_Write( (0xC0 + pos[1] & 0xF0) >> 4 ); LCD_Write( 0xC0 + pos[1] & 0x0F); } while ( 0 ); } else if ( pos[0] == 2 ) { do { LCD_Write( (0x94 + pos[1] & 0xF0) >> 4 ); LCD_Write( 0x94 + pos[1] & 0x0F); } while ( 0 ); } else if ( pos[0] == 3 ) { do { LCD_Write( (0xD4 + pos[1] & 0xF0) >> 4 ); LCD_Write( 0xD4 + pos[1] & 0x0F); } while ( 0 ); } } while ( 0 );
    LCD_putc(0);
}
