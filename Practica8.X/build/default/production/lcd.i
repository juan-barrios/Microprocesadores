...
void LCD_Custom_Char(unsigned char loc, const unsigned char *bitmap) {
    ...
    if(loc < 8) {
        ...
        do { LCD_Write( (0x40 + (loc * 8) & 0xF0) >> 4 ); LCD_Write( 0x40 + (loc * 8) & 0x0F); } while ( 0 );

        for(i = 0; i < 8; i++) {
            *(lcd.PORT) |= 1 << lcd.RS;
            LCD_Out((bitmap[i] & 0xF0) >> 4);

            *(lcd.PORT) |= 1 << lcd.EN;
            _delay((unsigned long)((40)*(8000000/4000000.0)));
            *(lcd.PORT) &= ~(1 << lcd.EN);

            LCD_Out(bitmap[i] & 0x0F);
    ...
    do { LCD_Write( (0x80 & 0xF0) >> 4 ); LCD_Write( 0x80 & 0x0F); } while ( 0 );
}
